...
public class AuthServiceImpl implements AuthService {
    ...
    public String login(LoginModel loginModel) {
        ...
        return getTokenClaims(user.getUserId(), rolesById.getData());
    }
    ...
    private String register(RegisterModel registerModel, List<Integer> roleIds) {
        ...
        return getTokenClaims(insert.getData().getUserId(), list);
    }


    private String getTokenClaims(int userId, List<RoleDTO> role) {
        HashMap<String, Object> claims = new HashMap<>(3);
        claims.put("userId", userId);
        List<String> values = role.stream().map(RoleDTO::getValue).collect(Collectors.toList());
